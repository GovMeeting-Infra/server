...
describe('ministryScope', () => {
  ...
  it('leaves the top role unscoped', () => {
    expect(ministryScope({ systemRole: 'SUPER_ADMIN', ministryId: null })).toEqual({});
  });

  it.each([
    ['undefined', undefined],
    ['null', null],
  ])('gives a ministry-less %s user a filter that matches nothing', (_label, ministryId) => {
...
describe('assertSameMinistry', () => {
  it('allows a match and the top role', () => {
    expect(() => assertSameMinistry({ systemRole: 'STAFF', ministryId: 'a' }, 'a')).not.toThrow();
    expect(() => assertSameMinistry({ systemRole: 'SUPER_ADMIN', ministryId: null }, 'a')).not.toThrow();
  });

  it('refuses across ministries, and refuses a ministry-less ordinary user', () => {
    expect(() => assertSameMinistry({ systemRole: 'STAFF', ministryId: 'a' }, 'b')).toThrow();
    expect(() => assertSameMinistry({ systemRole: 'MINISTER', ministryId: null }, 'b')).toThrow();
  });
});
